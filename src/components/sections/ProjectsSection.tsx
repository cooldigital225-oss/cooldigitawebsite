                ...
                <div>
                  ...
                  <div className="relative h-56 overflow-hidden bg-slate-100">
                    ...
                    <div className="absolute top-3 left-3">
                      {isReal ? (
                        <span className="px-2.5 py-1 rounded-lg text-[10px] font-extrabold uppercase bg-emerald-600 text-white shadow-xs tracking-wider">
                          PROJET RÉALISÉ
                        </span>
                      ) : (
                    ...
                    <div className="absolute bottom-3 left-3 right-3 bg-slate-900/90 backdrop-blur-xs text-white text-xs font-medium px-3 py-1.5 rounded-lg border border-white/10 flex items-center justify-between">
                      <span className={`font-bold text-[11px] ${isReal ? 'text-emerald-400' : 'text-amber-300'}`}>
                        {project.projectType}
                      </span>
                      <span className="text-[10px] bg-slate-800 text-slate-200 px-2 py-0.5 rounded font-semibold border border-slate-700 truncate max-w-[200px]">
          ...
          <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200 p-6 md:p-8 relative">
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2">
              {selectedProject.projectType === 'Projet réalisé' ? (
                <span className="px-2.5 py-1 rounded-md text-[10px] font-extrabold uppercase bg-emerald-600 text-white tracking-wide">
                  PROJET RÉALISÉ
                </span>
              ) : (
            ...
            <div className="space-y-5 text-sm text-slate-700 mb-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-slate-700">
                    {selectedProject.featureCategories.map((cat, idx) => (
                      <div key={idx} className="p-2.5 rounded-lg bg-white border border-slate-200">
                        <span className="font-bold text-slate-900 block mb-1 text-[11px] text-emerald-700">{cat.category}</span>
              ...
              {selectedProject.benefits && selectedProject.benefits.length > 0 ? (
                <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200">
                  ...
                  <p className="text-[11px] text-slate-500 italic mt-3 pt-2 border-t border-emerald-200/60">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-100">
              ...
              <Button
                ...
                onClick={() => {
                  setSelectedProject(null);
                  if (onNavigate) {
                    onNavigate('/services/logiciels-gestion-sur-mesure');
                  }
                }}
                icon={<ArrowUpRight className="w-4 h-4" />}
              >
